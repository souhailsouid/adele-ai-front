...
class FMPClient {
   ...
   * @param {string} query - Nom de l'entreprise à rechercher (ex: "apple")
   * @returns {Promise<Array>} Liste des entreprises correspondantes avec leur symbole
   */
  async searchCompanyByName(query) {
    if (!query || query.trim() === "") {
      throw new Error("Query parameter is required");
    }
    const data = await this.request(`/search-name`, { query: query.trim() });
   ...
   * @param {string} symbol - Symbole boursier (ex: "AAPL")
   * @returns {Promise<Object|null>} Données de cotation (prix, volume, variation, etc.)
   */
  ...
  async getEarningsSurprises(symbol) {
    return this.request(`/earnings-surprises`, { symbol });
  }

  /**
   * Estimations d'earnings (Earnings Report)
   * @param {string} symbol - Symbole boursier
   */
   ...
   * @param {string} timeframe - "1day", "1hour", "5min", etc. (défaut: "1day")
   */
  async getRSI(symbol, period = 14, timeframe = "1day") {
    return this.request(`/technical-indicators/rsi`, { symbol, periodLength: period, timeframe });
  }
  ...
  async getMACD(symbol, timeframe = "1day") {
    return this.request(`/technical-indicators/macd`, { symbol, timeframe });
  }

  /**
   * SMA (Simple Moving Average)
   * @param {string} symbol - Symbole boursier
   * @param {number} period - Période (défaut: 50)
   * @param {string} timeframe - "1day", "1hour", "5min", etc. (défaut: "1day")
   */
  async getSMA(symbol, period = 50, timeframe = "1day") {
    return this.request(`/technical-indicators/sma`, { symbol, periodLength: period, timeframe });
  }
   ...
   * @param {string} timeframe - "1day", "1hour", "5min", etc. (défaut: "1day")
   */
  async getEMA(symbol, period = 50, timeframe = "1day") {
    return this.request(`/technical-indicators/ema`, { symbol, periodLength: period, timeframe });
  }
   ...
   * @param {number} limit - Nombre de résultats (défaut: 50)
   * @param {number} page - Numéro de page (défaut: 0)
   */
  async getMarketNews(symbol = null, limit = 50, page = 0) {
    if (symbol) {
      // Utiliser l'endpoint de recherche par symbole
      return this.request("/news/stock", { symbols: symbol, page, limit });
    }
    // Utiliser l'endpoint des dernières actualités
    return this.request("/news/stock-latest", { page, limit });
  }
   ...
   * @param {string} symbol - Symbole boursier (ex: "AAPL")
   * @returns {Promise<Object|null>} Informations détaillées sur l'entreprise (capitalisation, secteur, PDG, etc.)
   */
   ...
   * @param {string} symbol - Symbole boursier (ex: "AAPL")
   * @param {string} period - Période: "annual" ou "quarter" (défaut: "annual")
   * @param {number} limit - Nombre de périodes à retourner (défaut: 5)
   * @returns {Promise<Array>} Liste des comptes de résultat
   */
  ...
  /**
   * Données fondamentales annuelles (alias pour compatibilité)
   * @deprecated Utilisez getIncomeStatement à la place
   */
  async getAnnualFundamentals(symbol) {
    return this.getIncomeStatement(symbol, "annual", 5);
  }
   ...
   * @param {string} symbol - Symbole boursier (ex: "AAPL")
   * @param {string} period - Période: "annual" ou "quarter" (défaut: "annual")
   * @param {number} limit - Nombre de périodes (défaut: 5)
   */
  ...
  /**
   * Estimations d'analystes (Analyst Estimates)
   * @param {string} symbol - Symbole boursier
   * @param {string} period - "annual" ou "quarter"
   * @param {number} page - Numéro de page (défaut: 0)
   * @param {number} limit - Nombre de résultats (défaut: 10, max: 10 pour plan Starter)
   */
  async getAnalystEstimates(symbol, period = "annual", page = 0, limit = 10) {
    // Limiter à 10 pour le plan Starter (limite de l'API)
    const maxLimit = Math.min(limit, 10);
    return this.request(`/analyst-estimates`, { symbol, period, page, limit: maxLimit });
  }
  ...
  /**
   * Transactions d'insiders (Insider Trades)
   * @param {string} symbol - Symbole boursier (optionnel, null pour toutes)
   * @param {number} limit - Nombre de résultats (défaut: 50)
   * @param {number} page - Numéro de page (défaut: 0)
   */
  async getInsiderTrades(symbol = null, limit = 50, page = 0) {
    // Limiter à 100 pour le plan Starter (limite de l'API)
    const maxLimit = Math.min(limit, 100);
    if (symbol) {
      // Utiliser l'endpoint de recherche par symbole
      return this.request(`/insider-trading/search`, { symbol, page, limit: maxLimit });
    }
    // Utiliser l'endpoint des dernières transactions
    return this.request(`/insider-trading/latest`, { page, limit: maxLimit });
  }

  /**
   * Transcripts des earnings (Earnings Transcript)
   * @param {string} symbol - Symbole boursier
   * @param {number} year - Année (ex: 2024)
   * @param {number} quarter - Trimestre: 1, 2, 3, ou 4 (défaut: 1)
   */
  async getEarningsTranscript(symbol, year, quarter = 1) {
    return this.request(`/earning-call-transcript`, { symbol, year, quarter });
  }

  /**
   * Documents SEC (SEC Filings)
   * @param {string} symbol - Symbole boursier
   * @param {string} type - Type de document: "10-K", "10-Q", "8-K", etc. (optionnel)
   * @param {string} from - Date de début (YYYY-MM-DD) (optionnel)
   * @param {string} to - Date de fin (YYYY-MM-DD) (optionnel)
   * @param {number} page - Numéro de page (défaut: 0)
   * @param {number} limit - Nombre de résultats (défaut: 100)
   */
  async getSECFilings(symbol, type = null, from = null, to = null, page = 0, limit = 100) {
    if (type) {
      // Utiliser l'endpoint de recherche par type de formulaire
      const params = { formType: type, page, limit };
      if (from) params.from = from;
      if (to) params.to = to;
      return this.request(`/sec-filings-search/form-type`, params);
    }
    // Utiliser l'endpoint de recherche par symbole
    const params = { symbol, page, limit };
    if (from) params.from = from;
    if (to) params.to = to;
    return this.request(`/sec-filings-search/symbol`, params);
  }

  /**
   * Transactions des sénateurs US (Senate Trading)
   * @param {string} symbol - Symbole boursier (optionnel, null pour toutes)
   * @param {number} page - Numéro de page (défaut: 0)
   * @param {number} limit - Nombre de résultats (défaut: 100)
   */
  async getSenateTrading(symbol = null, page = 0, limit = 100) {
    if (symbol) {
      return this.request(`/senate-trades`, { symbol });
    }
    // Retourner les dernières transactions
    return this.request(`/senate-latest`, { page, limit });
  }
  ...
  async getESGScore(symbol) {
    return this.request(`/esg-ratings`, { symbol });
  }
  ...
  async getSP500Constituents() {
    return this.request(`/sp500_constituent`);
  }

  /**
   * Liste des actions par bourse
   * @param {string} exchange - "NASDAQ", "NYSE", "AMEX"
   */
  async getStockList(exchange = null) {
    const params = {};
    if (exchange) params.exchange = exchange;
    return this.request(`/stock/list`, params);
  }
