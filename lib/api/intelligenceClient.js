...
class IntelligenceClient extends BaseApiClient {
  ...
  /**
   * Analyse LLM "Five Factors" pour les options (qualitative, non-directionnelle)
   * @param {string} ticker - Symbole boursier (ex: "NVDA")
   * @param {boolean} refresh - Rafraîchir les données (défaut: true)
   * @returns {Promise<Object>} Réponse avec success, ticker, data, analysis, timestamp
   */
  async getOptionsFiveFactors(ticker, refresh = true) {
    return this.requestMain(`/ai/options-five-factors`, {
      method: 'POST',
  ...
  /**
   * Analyse LLM complète des options (utilise getOptionsFiveFactors en interne)
   * @param {string} ticker - Symbole boursier (ex: "NVDA")
   * @param {Object} payload - Réponse complète de getOptionsFiveFactors (optionnel, sera récupéré si non fourni)
   * @param {boolean} refresh - Rafraîchir les données pour getOptionsFiveFactors (défaut: true)
   * @returns {Promise<Object>} Réponse avec success, ticker, analysis (bias, signal_quality, contradictions, key_levels, report), timestamp
   */
  async getOptionsAnalysis(ticker, payload = null, refresh = true) {
    // Si payload n'est pas fourni, on récupère les données via getOptionsFiveFactors
    if (!payload) {
      payload = await this.getOptionsFiveFactors(ticker, refresh);
    }

    return this.requestMain(`/ai/options-analysis`, {
      method: 'POST',
