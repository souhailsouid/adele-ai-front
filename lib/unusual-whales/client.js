...
class UnusualWhalesClient {
  constructor() {
    ...
    // On est plus conservateur : 1 requête par seconde = 60 req/min (bien en dessous de 120)
    this.maxCallsPerSecond = 1; // 1 requête par seconde max
    this.lastCallTime = 0;
    this.minDelayBetweenCalls = 1000; // 1 seconde entre chaque appel
    
    // Gestion du rate limiting basé sur les headers de l'API
    this.remainingRequests = 120; // Par défaut, on assume 120 requêtes disponibles
    this.resetTime = null; // Timestamp de reset
  ...
  async throttle() {
    ...
    // Vérifier si on doit attendre le reset
    if (this.resetTime && now < this.resetTime) {
      const waitTime = this.resetTime - now;
      console.warn(`⏳ Rate limit: waiting ${Math.ceil(waitTime / 1000)}s before next request`);
      await new Promise((resolve) => setTimeout(resolve, waitTime + 100)); // +100ms de marge
      this.resetTime = null;
      this.remainingRequests = 120; // Reset après l'attente
    }
    
    // Vérifier si on a encore des requêtes disponibles
    if (this.remainingRequests <= 0) {
      const waitTime = this.resetTime ? (this.resetTime - now) : 60000; // 60s par défaut
    ...
    // Throttle basique entre les appels
  ...
  async request(endpoint, params = {}) {
    ...
    try {
      ...
      // Lire les headers de rate limiting
      const remainingHeader = response.headers.get("x-uw-req-per-minute-remaining");
      const resetHeader = response.headers.get("x-uw-req-per-minute-reset");
      
      if (remainingHeader !== null) {
        this.remainingRequests = parseInt(remainingHeader, 10) || 0;
      }
      
      if (resetHeader !== null) {
        const resetMs = parseInt(resetHeader, 10);
        if (resetMs) {
          this.resetTime = Date.now() + resetMs;
        }
      }
      ...
      if (!response.ok) {
        ...
        // Gestion spéciale pour les erreurs 429 (rate limit)
        if (response.status === 429) {
          const resetMs = parseInt(resetHeader, 10) || 60000; // 60s par défaut
          this.resetTime = Date.now() + resetMs;
          this.remainingRequests = 0;
          
          const waitSeconds = Math.ceil(resetMs / 1000);
          const errorMessage = `Rate limit reached (${this.remainingRequests} remaining). Reset in ${waitSeconds}s.`;
          const apiError = new Error(errorMessage);
          apiError.status = 429;
          apiError.resetTime = this.resetTime;
          apiError.remainingRequests = this.remainingRequests;
          throw apiError;
        }
