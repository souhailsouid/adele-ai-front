/**
 * Composant pour l'analyse LLM enrichie de l'activité institutionnelle d'un ticker
 */
...
function TickerInstitutionalAnalysis({ ticker, onAnalysisComplete }) {
  ...
  useEffect(() => {
    ...
    const loadAnalysis = async () => {
      try {
        setLoading(true);
        setError(null);
        const response = await intelligenceClient.getTickerInstitutionalAnalysis(ticker);
        ...
        console.error("Error loading ticker institutional analysis:", err);
      ...
      <Card>
        <MDBox p={3}>
          <MDTypography variant="body2" color="text.secondary">
            Aucune analyse institutionnelle disponible pour {ticker}
          </MDTypography>
        </MDBox>
      </Card>
    );
        ...
        <MDBox display="flex" justifyContent="space-between" alignItems="center" mb={3}>
          <MDBox>
            <MDTypography variant="h5" fontWeight="bold" mb={0.5}>
              Analyse Institutionnelle - {ticker}
            </MDTypography>
...
export default TickerInstitutionalAnalysis;
