...
function GreekExposureByStrike({ ticker = "", date = "" }) {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        ...
        const response = await unusualWhalesClient.getStockGreekExposureByStrike(ticker, params);
        ...
        console.error("Error loading greek exposure by strike:", err);
      ...
      <MDBox p={3}>
        <MDBox display="flex" justifyContent="space-between" alignItems="center" mb={2}>
          <MDTypography variant="h6">
            Greek Exposure by Strike ({ticker}) ({data.length})
          </MDTypography>
          <Tooltip title="Exposition aux grecs groupée par strike price">
