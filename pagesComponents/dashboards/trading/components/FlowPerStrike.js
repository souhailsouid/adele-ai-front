...
function FlowPerStrike({ ticker = "", date = "" }) {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        ...
        const response = await unusualWhalesClient.getStockFlowPerStrike(ticker, params);
        const extracted = Array.isArray(response) ? response : (response?.data || []);
        setData(extracted);
      } catch (err) {
        console.error("Error loading flow per strike:", err);
      ...
      <MDBox p={3}>
        <MDBox display="flex" justifyContent="space-between" alignItems="center" mb={2}>
          <MDTypography variant="h6">
            Flow per Strike ({ticker}) ({data.length})
          </MDTypography>
          <Tooltip title="Option flow par strike pour un jour de trading donné">
            ...
            <Table size="small">
              <MDBox component="thead">
                <TableRow>
                  ...
                  <DataTableHeadCell width="18%" align="left">Date</DataTableHeadCell>
                </TableRow>
              </MDBox>
              <TableBody>
                {data.map((item, index) => {
